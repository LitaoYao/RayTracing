...
vec3 random_in_hemisphere(const vec3& noraml)
{
	vec3 in_unit_sphere = random_in_unit_sphere();
	if (dot(in_unit_sphere, noraml) > 0.0) // In the same hemisphere as the normal
	{
...
{
	// n*cos(theta) = - v_กฮ
	// v_กอ' = etai_over_etat * v_กอ = etai_over_etat * (v - v_กฮ) = etai_over_etat * (v + n * cos(theta)) 
	// dot(n, -v) = |n| * |-v| * cos(theta) = cos(theta)
